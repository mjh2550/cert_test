#ifndef _KS_BIGINT_H_
#define _KS_BIGINT_H_
...
KS_WORD     Big_Add                 ( BigInt *x, BigInt *y, BigInt *z); 
...
KS_WORD		Big_DerDecode			( BigInt *x, KS_BYTE *y, int z );
KS_WORD		Big_DerEncode			( KS_BYTE *x, BigInt *y, int *z );
KS_WORD		Big_DerLength			( BigInt *x );
KS_WORD     Big_DivByWord           ( BigInt *x, KS_WORD *r, BigInt *y, KS_WORD z);             // x = y / z
KS_WORD     Big_Divide              ( BigInt *x, BigInt *r, BigInt *y, BigInt *z);              // y = x * z + r
KS_WORD     Big_ExtendedGCD         ( BigInt *z, BigInt *a, BigInt *b, BigInt *x, BigInt *y);	// ax + by = gcd ( x, y)
void        Big_Free                ( BigInt *x);                                               // memory free
KS_WORD     Big_GCD                 ( BigInt *x, BigInt *y, BigInt *z);                         // x = gcd ( y, z)
...
KS_WORD		Big_ModExp  			( BigInt *x, BigInt *y, BigInt *z, BigInt *w);				// x = y^z  ( mod w )
KS_WORD		Big_ModExpMont			( BigInt *x, BigInt *y, BigInt *z, BigInt *w);
KS_WORD		Big_ModExpWindow		( BigInt *x, BigInt *y, BigInt *z, BigInt *w);
KS_WORD		Big_ModExpWindowMont	( BigInt *x, BigInt *y, BigInt *z, BigInt *w);				// z = x^y mod m ( slide-window mwthod )
...
KS_WORD     Big_Sub                 ( BigInt *x, BigInt *y, BigInt *z);                         // x = y - z

int			Big_ModMul				(BigInt *result, BigInt *inputA, BigInt *inputB, BigInt *mod );	// x = y*z  ( mod w )
KS_WORD		Big_GetFilledBitNum		( BigInt *a);
